<template>
  <div>
    ...
    />
    <MechanicsAction />
    <DefinitionCategoryPrize :slides="prizes" />
    <PrizesForByers
      :guaranteePrize="guaranteePrize"
      :mainPrize="mainPrize"
      :slides="slides"
    />
    <InformationPoints />
...
import PrizesForByers from "@/components/promo/PrizesForByers.vue";
...
export default {
  ...
  methods: {
    ...
    handleMainPage() {
      this.$router.push("/");
      gtag("event", "come_back", { event_category: "click_button" }); // eslint-disable-line
    },
    handleRegisterCheck() {
      if (!this.isAuth) {
